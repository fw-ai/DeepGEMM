...
def get_source_git_commit() -> str:
    """Return the exact source OID embedded in the native extension.

    Release builders without a ``.git`` directory must provide the same value
    explicitly.  A shortened or symbolic revision is deliberately rejected:
    consumers use this value as a fail-closed ABI/provenance boundary.
    """
    revision = os.environ.get('DEEP_GEMM_GIT_COMMIT')
    if revision is None:
        revision = subprocess.check_output(
            ['git', '-C', os.path.dirname(os.path.realpath(__file__)), 'rev-parse', 'HEAD'],
            text=True,
        ).strip()
    if re.fullmatch(r'[0-9a-f]{40}', revision) is None:
        raise RuntimeError(
            'DEEP_GEMM_GIT_COMMIT must be the full 40-character lowercase source OID; '
            f'got {revision!r}'
        )
    return revision


SOURCE_GIT_COMMIT = get_source_git_commit()
GIT_COMMIT_DEFINE = f'-DDEEP_GEMM_GIT_COMMIT_TOKEN={SOURCE_GIT_COMMIT}'
...
cxx_flags = ['-std=c++17', '-O3', '-fPIC', '-Wno-psabi', '-Wno-deprecated-declarations',
             ...
             GIT_COMMIT_DEFINE]
nvcc_flags = [
    '-std=c++17', '-O3', '--expt-relaxed-constexpr', '--expt-extended-lambda',
...
sources = ['csrc/python_api.cpp', 'csrc/sm103_fp8_block128.cu']
build_include_dirs = [
    f'{CUDA_HOME}/include',
    f'{CUDA_HOME}/include/cccl',
    os.path.join(current_dir, 'deep_gemm/include'),
    os.path.join(current_dir, 'third-party/cutlass/include'),
    os.path.join(current_dir, 'third-party/cutlass/tools/util/include'),
