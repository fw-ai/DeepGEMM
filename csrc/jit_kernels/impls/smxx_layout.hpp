...
static torch::Tensor get_mn_major_tma_aligned_packed_ue8m0_tensor(const torch::Tensor& sf,
    ...
    if (use_strided_psum_scales) {
        constexpr int block_mn = 48, num_threads = 512;
        const int sf_layout = batched_sf.stride(1) == 1 and batched_sf.stride(2) % 4 == 0 and
            (reinterpret_cast<uint64_t>(batched_sf.data_ptr()) & 0xfu) == 0 ? 1 : 2;
        const auto kernel = jit->compile("transpose_and_pack_strided_fp32_into_ue8m0", std::format(R"(
#include <deep_gemm/impls/smxx_layout.cuh>
using namespace deep_gemm;
static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(&transpose_and_pack_strided_fp32_into_ue8m0<{}, {}, {}, {}, true, {}>);
}}
)", num_threads, block_mn, sf_k, num_psum_groups, sf_layout));
        jit->launch(kernel, {
            .num_smem_bytes = block_mn * sf_k * 4 + align(num_psum_groups * 2, 4) * 4,
            .grid_dim = dim3(ceil_div(mn, block_mn), 1, 1),
            .block_dim = dim3(num_threads, 1, 1),
        }, batched_sf.data_ptr(), out.data_ptr(), static_cast<uint32_t>(mn),
           static_cast<uint64_t>(batched_sf.stride(1)), static_cast<uint64_t>(batched_sf.stride(2)),
           psum_layout->data_ptr(), static_cast<uint32_t>(m_alignment));
