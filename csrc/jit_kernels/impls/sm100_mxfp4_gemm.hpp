...
static void sm100_mxfp4_gemm(const torch::Tensor& a, const torch::Tensor& sfa,
    ...
    const auto tensor_map_b = make_tma_2d_desc(b, k, n, block_k, load_block_n,
                                               ...
                                               swizzle_ab, 0, false, false);
    // UE8M0 SF descriptors (MN-major, no swizzle)
    const auto tensor_map_sfa = make_tma_sf_desc(cute::UMMA::Major::MN, sfa, m, k, block_m, gran_k, 1, 0);
    const auto tensor_map_sfb = make_tma_sf_desc(cute::UMMA::Major::MN, sfb, n, k, block_n, gran_k, 1, 0);
    // BF16 output (N-major)
    ...
    const int smem_sfa = sf_block_m * static_cast<int>(sizeof(int));
    const int smem_sfb = sf_block_n * static_cast<int>(sizeof(int));
