...
static std::tuple<int, int, int, int, int> get_block_config_for_mega_moe(
    ...
    if (mma_kind == MmaKind::BF16)
        block_m = deep_jit::get_env<int>("DG_BF16_MEGA_MOE_BLOCK_M", block_m);
    int store_block_m = block_m <= 16 ? 8 : block_m <= 96 ? 16 : block_m <= 192 ? 32 : 40;
    int block_k = 128 / get_num_mma_elem_bytes(mma_kind);
    if (mma_kind == MmaKind::BF16) {
        store_block_m = deep_jit::get_env<int>("DG_BF16_MEGA_MOE_STORE_BLOCK_M", store_block_m);
        block_k = deep_jit::get_env<int>("DG_BF16_MEGA_MOE_BLOCK_K", block_k);
    }
    ...
    const int epilogue_warpgroups = mma_kind == MmaKind::BF16
        ? deep_jit::get_env<int>("DG_BF16_MEGA_MOE_EPILOGUE_WARPGROUPS", 2) : 2;
    DG_HOST_ASSERT(epilogue_warpgroups > 0 and epilogue_warpgroups <= 4);
    DG_HOST_ASSERT(store_block_m > 0 and block_m % (store_block_m * epilogue_warpgroups) == 0);
    return {2, block_m, store_block_m, block_k, epilogue_warpgroups * 128};
}
...
static std::pair<int, int> get_pipeline_config_for_mega_moe(
    ...
    if (mma_kind == MmaKind::BF16) {
        num_stages = deep_jit::get_env<int>(
