...
sm100_fp8_fp4_mega_moe_impl(void* y,
    ...
    const auto combine_token_buffer = layout::Buffer(
        ...
        layout::align_buffer_base(l2_sf_buffer.get_end_ptr(), 128u)
    );
                    ...
                    for (uint32_t i = 0; i < kNumAtomsPerStore; ++ i) {
                        ...
                        for (uint32_t k = 0; k < 2; ++ k) {
                            ...
                            if constexpr (kSaveL1Preact) {
                                ...
                                // The forward weights and TMEM accumulator
                                // use MegaMoE's eight-column gate/up
                                // interleave. Save the training preactivation
                                // in canonical [all gate | all up] order so
                                // backward can overwrite it in place without
                                // a full-pool deinterleave pass.
                                const uint32_t physical_gate_col =
                                    output_col(gate_col);
                                const uint32_t branch_col =
