...
// Publish the two L1 side-LoRA dgrad branches without a [pool, hidden]
// temporary. The base wave has already populated the fixed local/remote
// combine slots; every valid expert-pool row owns exactly one such slot, so
// the split-additive BF16 update is race-free and needs no atomics.
template <
    uint32_t kHidden, uint32_t kNumExperts, uint32_t BLOCK_M,
    uint32_t kNumRanks, uint32_t kNumSMs, bool kWriteGradXPool,
    bool kDirectRemoteGradX>
CUTLASS_GLOBAL void sm100_bf16_mega_moe_side_lora_grad_x_impl(
    ...
    if constexpr (kDirectRemoteGradX) {
        ...
        // overwritten below, but masked top-k slots have no expert owner and
        ...
        for (uint64_t vector_idx =
            ...
            reinterpret_cast<uint4*>(combine_buffer)[vector_idx] =
                make_uint4(0, 0, 0, 0);
        }
        comm::nvlink_barrier<kNumRanks, kNumSMs, 256, 1, 73>(
            workspace, sym_buffer, blockIdx.x, threadIdx.x,
            []() { __syncthreads(); });
    }

    uint32_t pool_block_offset = 0;
    uint32_t global_tile_offset = 0;
    #pragma unroll
    for (uint32_t expert_idx = 0; expert_idx < kNumExperts;
        ...
        const uint32_t num_m_blocks =
            math::ceil_div(num_tokens, BLOCK_M);
        const uint32_t num_n_blocks = kHidden / kOutputTileN;
        const uint32_t num_tiles = num_m_blocks * num_n_blocks;
        uint32_t first_global_tile = blockIdx.x;
        if (first_global_tile < global_tile_offset) {
            first_global_tile += math::ceil_div(
                global_tile_offset - first_global_tile,
                static_cast<uint32_t>(gridDim.x)) * gridDim.x;
        }
        for (uint32_t local_tile =
                 first_global_tile < global_tile_offset + num_tiles
                     ? first_global_tile - global_tile_offset
                     : num_tiles;
             local_tile < num_tiles;
            ...
            for (uint32_t linear = threadIdx.x;
                 linear < BLOCK_M * kVectorsPerTile;
                 linear += blockDim.x) {
                const uint32_t local_m = linear / kVectorsPerTile;
                const uint32_t local_vector =
                    linear - local_m * kVectorsPerTile;
                const uint32_t expert_row = m_block * BLOCK_M + local_m;
                if (expert_row >= num_tokens)
                    continue;
                const uint32_t pool_row =
                    pool_block_offset + expert_row;
                if (pool_row >= num_pool_rows)
                    continue;
                ...
                if constexpr (kDirectRemoteGradX) {
                    ...
                    auto* local_dst = combine_buffer +
                        ((static_cast<uint64_t>(metadata.topk_idx) *
        ...
        global_tile_offset += num_tiles;
        pool_block_offset += num_m_blocks * BLOCK_M;
    }
    if constexpr (kNumRanks > 1 && kDirectRemoteGradX) {
        comm::nvlink_barrier<kNumRanks, kNumSMs, 256, 2, 74>(
            workspace, sym_buffer, blockIdx.x, threadIdx.x,
            []() { __syncthreads(); });
    }
#endif
...
CUTLASS_GLOBAL void sm100_bf16_mega_moe_side_lora_axpy2_impl(
    ...
    for (uint64_t vector_idx =
         ...
         vector_idx < num_vectors;
         vector_idx += static_cast<uint64_t>(kNumSMs) * blockDim.x) {
        BF16x8 d, s1, s3;
    ...
    for (uint64_t idx = num_vectors * kVectorElems +
             static_cast<uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
         idx < num_elements;
         idx += static_cast<uint64_t>(kNumSMs) * blockDim.x) {
        const bf16 side1(scale * static_cast<float>(src1[idx]));
        const bf16 side3(scale * static_cast<float>(src3[idx]));
        dst[idx] = bf16(static_cast<float>(dst[idx]) +
                        static_cast<float>(side1) +
                        static_cast<float>(side3));
    }
#endif
}
...
CUTLASS_GLOBAL void sm100_bf16_mega_moe_side_lora_scale_grads_impl(
    const __grid_constant__ MegaMoESideLoraBackwardParams side_lora) {
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 1000)) || defined(__CLION_IDE__)
    using bf16 = cutlass::bfloat16_t;
    constexpr uint64_t kRank = 128;
    // A1, A3, and B2 are shared across experts by the adapter layout.
    constexpr uint64_t kHiddenElems =
        static_cast<uint64_t>(kHidden) * kRank;
    constexpr uint64_t kIntermediateElems =
        static_cast<uint64_t>(kNumExperts) * kIntermediateHidden * kRank;
    constexpr uint64_t kVectorElems = 8;
    constexpr uint64_t kHiddenVectors = kHiddenElems / kVectorElems;
    constexpr uint64_t kIntermediateVectors =
        kIntermediateElems / kVectorElems;
    constexpr uint64_t kScaledVectors =
        3 * kHiddenVectors + 3 * kIntermediateVectors;
    union alignas(16) BF16x8 {
        uint4 packed;
        bf16 element[8];
    };
    for (uint64_t linear =
             static_cast<uint64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
         linear < kScaledVectors;
         linear += static_cast<uint64_t>(gridDim.x) * blockDim.x) {
        uint4* ptr;
        uint64_t offset = linear;
        if (offset < kHiddenVectors) {
            ptr = reinterpret_cast<uint4*>(side_lora.grad_a1) + offset;
        } else if ((offset -= kHiddenVectors) < kIntermediateVectors) {
            ptr = reinterpret_cast<uint4*>(side_lora.grad_b1) + offset;
        } else if ((offset -= kIntermediateVectors) < kHiddenVectors) {
            ptr = reinterpret_cast<uint4*>(side_lora.grad_a3) + offset;
        } else if ((offset -= kHiddenVectors) < kIntermediateVectors) {
            ptr = reinterpret_cast<uint4*>(side_lora.grad_b3) + offset;
        } else if ((offset -= kIntermediateVectors) < kIntermediateVectors) {
            ptr = reinterpret_cast<uint4*>(side_lora.grad_a2) + offset;
        } else {
            offset -= kIntermediateVectors;
            ptr = reinterpret_cast<uint4*>(side_lora.grad_b2) + offset;
        }
        BF16x8 values;
        values.packed = *ptr;
        #pragma unroll
        for (int i = 0; i < 8; ++i) {
            values.element[i] = bf16(
                static_cast<float>(values.element[i]) * side_lora.scale);
        }
        *ptr = values.packed;
    }
#endif
...
sm100_bf16_mega_moe_side_lora_backward_wave_impl(
    ...
    constexpr uint32_t kTraceSiteCount = 23;
    ...
    if constexpr (!kBF16Mode && !kGateUpPrepared) {
      if (warp_idx == 0) {
    ...
    if (
        ...
        constexpr uint32_t kNumDispatchRegisters =
            kBF16Mode ? 56 : (kGateUpPrepared ? 40 : 48);
...

    {
                ...
                [&](const uint32_t& expert_idx,
                    ...
                    for (uint32_t s = 0;
                        ...
                        for (uint32_t linear =
                            ...
                            // Consume the rank-width contraction directly at
                            // the base W2 dgrad epilogue. Both branches are
                            // rounded to BF16 before their sum, matching an
                            // explicit base + side-LoRA graph without ever
                            // materializing a [pool, intermediate] side dgrad.
                            // The dedicated host path tensor-core-expands
                            // t2 @ A2 into this otherwise-uninitialized output
                            // plane. Consume it once, then overwrite it below
                            // with the combined base+side grad_h.
                            const float side_grad_h_accum =
                                static_cast<float>(h_weighted_output[
                                    static_cast<uint64_t>(pool_row) *
                                        kIntermediateHidden + hidden_col]);
                            const cd_dtype_t side_grad_h_bf16 =
                                cd_dtype_t(side_lora.scale *
                                           side_grad_h_accum);
                            const cd_dtype_t combined_grad_h_w2 =
                                cd_dtype_t(
                                    static_cast<float>(grad_h_w2) +
                                    static_cast<float>(side_grad_h_bf16));
                            const float route_weight =
                                route_weights_fp32 != nullptr
                                ? route_weights_fp32[pool_row]
                                : static_cast<float>(
                            ...
                            const float neg_exp =
                                (!kBF16Mode && !kGateUpPrepared) || kFastMath
                                ? __expf(-z)
                                : expf(-z);
                            const float denom =
                            ...
                            if constexpr (
                                ...
                                // Native grouped experts materialize BF16
                                // clamp -> SiLU -> SiLU*up. Autograd likewise
                                // rounds grad_h*up before silu_backward.
                                const cd_dtype_t silu_bf16 =
                                    cd_dtype_t(gate / denom);
                                h_act_bf16 = cd_dtype_t(__fmul_rn(
                                    static_cast<float>(silu_bf16), up));
                                const cd_dtype_t grad_silu_bf16 =
                                    cd_dtype_t(__fmul_rn(grad_h, up));
                                const float one_minus_sig =
                                    __fsub_rn(1.0f, sig);
                                const float silu_inner = __fadd_rn(
                                    1.0f, __fmul_rn(gate, one_minus_sig));
                                const float grad_silu_sig = __fmul_rn(
                                    static_cast<float>(grad_silu_bf16), sig);
                                grad_gate_bf16 = cd_dtype_t(
                                    gate_in_range
                                    ? __fmul_rn(grad_silu_sig, silu_inner)
                                    : 0.0f);
                                grad_up_bf16 = cd_dtype_t(
                                    up_in_range
                                    ? __fmul_rn(
                                          grad_h,
                                          static_cast<float>(silu_bf16))
                                    : 0.0f);
                            } else {
                            ...
                            if constexpr (kBF16Mode) {
                                // The BF16 forward consumes W13 in MegaMoE's
                                // 8-row [gate, up] interleave. Emit dSwiGLU in
                                // that same K order so W13 dgrad can consume
                                // the already-live forward weight directly;
                                // no full expert-weight deinterleave is needed
                                // between recomputation and backward.
            ...
            if constexpr (
                ...
                constexpr uint32_t kRouteColumns =
                    kRouteWeightMode ==
                        ...
                        ? kHidden
                        : kIntermediateHidden;
                // The reference POST_DOWN path uses a Triton tl.sum with a
        ...
        if constexpr (
            kBF16Mode &&
            kRouteWeightMode == RouteWeightMode::PreDown) {
            // W13 dgrad consumes the forward-format 8-row [gate, up]
            // interleave above.  Once every CTA has drained those reads,
            // publish the two canonical activation-gradient planes directly
            // into backward buffers that are dead at this point.  Keeping
            // this conversion inside the persistent wave avoids the old
            // expert-weight deinterleave/cat and gives the rank-128 adapter
            // GEMMs ordinary contiguous operands.
            full_grid_phase_barrier(22);
            for (uint64_t linear =
                     static_cast<uint64_t>(blockIdx.x) *
                         kNumThreads +
                     threadIdx.x;
                 linear <
                     static_cast<uint64_t>(num_pool_rows) *
                         kIntermediateHidden;
                 linear +=
                     static_cast<uint64_t>(kNumSMs) *
                     kNumThreads) {
                const uint32_t pool_row =
                    linear / kIntermediateHidden;
                const uint32_t col =
                    linear -
                    static_cast<uint64_t>(pool_row) *
                        kIntermediateHidden;
                ...
                const uint64_t src_offset =
                    ...
                    interleaved_col;
                h_act_output[linear] =
                    grad_gate_up_output[src_offset];
                grad_h_output[linear] =
                    grad_gate_up_output[src_offset + 8];
            }
        }
