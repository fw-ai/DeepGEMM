...
template <
    ...
    bool kFastMath,
    // NVFP4 (E4M3 SF gran-16 + per-tensor global scale) vs MXFP4 (UE8M0 SF gran-32)
    bool kIsNVFP4 = false,
...
sm100_mxfp4_mxfp4_mega_moe_impl(void* y,
    ...
    // NOTES: MXFP4 packs 2 elements per byte, so token byte sizes are halved vs FP8.
    //        UE8M0 scale-factor layouts (gran-32, int32-packed) are unchanged.
    constexpr auto fp8_token_layout = layout::Data(kHidden / 2);
    constexpr auto bf16_token_layout = layout::Data(kHidden * sizeof(nv_bfloat16));
    constexpr auto fp8_intermediate_token_layout = layout::Data(kIntermediateHidden / 2);
    constexpr uint32_t kGranK = kIsNVFP4 ? 16 : 32;
    constexpr auto fp8_sf_layout = layout::Data(kHidden / kGranK);
    constexpr auto fp8_intermediate_sf_layout = layout::Data(kIntermediateHidden / kGranK);
    ...
    // Data types
    // NOTES: both activations and weights are packed MXFP4 (E2M1, 2 elems/byte)
    ...
    constexpr uint32_t UMMA_BLOCK_K_BYTES = UMMA_BLOCK_K / 2;
    constexpr uint32_t UMMA_K_BYTES = UMMA_K / 2;
    // SF count along K per token, in int32 units (4 SF bytes each): gran-32 -> BLOCK_K/128, gran-16 -> BLOCK_K/64.
    constexpr uint32_t kNumSFKUint32 = BLOCK_K / (kGranK * 4);
    // SF int32s per token per UMMA_BLOCK_K (=128): gran-32 -> 1, gran-16 -> 2.
    constexpr uint32_t kNumSFKUint32PerUmmaBlock = UMMA_BLOCK_K / (kGranK * 4);

    // Swizzle configs
    // NOTES: K-major packed FP4 uses a swizzle equal to the K extent in bytes (BLOCK_K/2)
    constexpr uint32_t kSwizzleAMode = BLOCK_K_BYTES;
    ...
    struct SharedStorage {
        ...
        union {
            // L1 output is packed FP4 (E2M1, 2 elems/byte). NOTES: over-sized to the FP8
            // footprint (the BF16 `l2` member dominates the union anyway) so the stage-1
            // placeholder store stays in-bounds; the real packed layout is finalized in stage 2.
        ...
        // Packed-FP4 A/B tiles: byte-addressed (`BLOCK_K / 2` bytes per row)
        ...
        uint32_t smem_sfa[kNumStages][SF_BLOCK_M * kNumSFKUint32];
        uint32_t smem_sfb[kNumStages][SF_BLOCK_N * kNumSFKUint32];
        ...
        // Cross-CTA "loads-done" barrier: per-CTA SM90 loads each fill their own CTA's
        // `full_barriers`; both CTAs' sync warp then funnel arrival to the leader so the
        // (leader-only) 2-SM MMA waits until BOTH CTAs' operands are resident.
        Barrier with_sf_full_barriers[kNumStages];
    ...
    // Each K-uint32 of SF occupies (SF_BLOCK/32) tmem cols (the MN rows; cell = 4 SF bytes).
    // The 2-bit `sf_id` field selects the SF WITHIN a K-uint32 (<=3); crossing K-uint32s
    // (gran-16 needs 2/K-block) is done via the SF tmem ADDRESS, not sf_id.
    constexpr uint32_t kSFAColsPerKUint = SF_BLOCK_M / 32;
    constexpr uint32_t kSFBColsPerKUint = SF_BLOCK_N / 32;
    constexpr uint32_t kNumSFATmemCols = kSFAColsPerKUint * kNumSFKUint32;
    constexpr uint32_t kNumSFBTmemCols = kSFBColsPerKUint * kNumSFKUint32;
    ...
    if (warp_idx == 0) {
        ...
        if (cute::elect_one_sync()) {
            ...
            for (uint32_t i = 0; i < kNumStages; ++ i) {
                // Per-CTA SM90 loads: A-load + B-load warps each arrive on their own CTA's full barrier
                shared_storage.full_barriers[i].init(2);
                shared_storage.empty_barriers[i].init(1);
                // Both CTAs' sync warp (32 lanes each) arrive on the leader CTA
                shared_storage.with_sf_full_barriers[i].init(2 * 32);
            }
            #pragma unroll
    ...
    if (warp_idx < kNumDispatchWarps) {
        ...
        for (uint32_t token_idx = sm_idx * kNumDispatchWarps + warp_idx; ; token_idx += kNumGlobalWarps) {
            ...
            // Packed-FP4 token bytes are divided into chunks (hidden / 2 bytes per token)
            constexpr uint32_t kNumTokenBytes = kHidden / 2;
            constexpr uint32_t kNumChunks = kNumTokenBytes / kNumBytesPerPull;
            DG_STATIC_ASSERT(kNumChunks * kNumBytesPerPull == kNumTokenBytes, "kNumBytesPerPull must divide packed token bytes");
        ...
        scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                // K-uint32 stride per K-block: gran-32 -> BLOCK_K/128, gran-16 -> BLOCK_K/64
                uint32_t sfa_k_idx = k_block_idx * kNumSFKUint32;

                // Add 2 CTA offsets for non-leader CTA
                if (not is_leader_cta)
                    ring_m_idx += scheduler.template get_valid_m<true>() / 2;

                // TMA copy tokens (TRUE 2-CTA cta_group::2) + SFA (per-CTA SM90), then arrive.
                // NOTES (validated against CUTLASS via instrumentation, and in the standalone GEMM):
                //   - acts use `SM100_TMA_2SM_LOAD` with the per-CTA M offset above; the 2-SM atom
                //     routes ALL data tx to the LEADER's `full` barrier (cluster peer-bit masking).
                //   - SFA uses a per-CTA SM90 load into THIS CTA's smem (the 2-CTA UTCCP reads both
                //     CTAs' SF), signaling THIS CTA's own `full`. So leader expects 2x data + own SF,
                //     while the non-leader's `full` only sees its own SF (its data went to the leader).
                //   - Packed FP4 is a single raw 2D TMA (the `tma::copy` byte-splitter overruns FP4).
                if (cute::elect_one_sync()) {
                    // acts: TRUE 2-CTA cta_group::2 load (routes tx to leader); use the ring offset
                    cute::SM100_TMA_2SM_LOAD_2D::copy(
                        tensor_map_a_ptr, reinterpret_cast<uint64_t*>(&shared_storage.full_barriers[stage_idx]),
                        static_cast<uint64_t>(cute::TMA::CacheHintSm100::EVICT_NORMAL),
                        shared_storage.smem_a[stage_idx], k_idx, ring_m_idx);
                    // SFA: per-CTA SM90 load into THIS CTA's smem (the 2-CTA UTCCP reads both CTAs' SF)
                    tma::copy<SF_BLOCK_M, 1, 0>(
                        tensor_map_sfa_ptr, &shared_storage.full_barriers[stage_idx], shared_storage.smem_sfa[stage_idx], sfa_ring_m_idx, sfa_k_idx);
                    // Leader collects BOTH CTAs' acts (2-SM routed) + its own SFA; non-leader sees only its SFA
                    if (is_leader_cta)
                        shared_storage.full_barriers[stage_idx].arrive_and_expect_tx(
                            sizeof(SharedStorage::smem_a[0]) * 2 + sizeof(SharedStorage::smem_sfa[0]));
                    else
                        shared_storage.full_barriers[stage_idx].arrive_and_expect_tx(
                            sizeof(SharedStorage::smem_sfa[0]));
                }
                __syncwarp();
            }
        });
        ...
        scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                uint32_t sfb_k_idx = local_expert_idx * shape_sfb_k + k_block_idx * kNumSFKUint32;

                // TMA copy weights (TRUE 2-CTA cta_group::2; both CTAs use the same n coord so each
                // loads the full BLOCK_N replica) + SFB (per-CTA SM90). Data tx routes to the leader.
                if (cute::elect_one_sync()) {
                    cute::SM100_TMA_2SM_LOAD_2D::copy(
                        tensor_map_b_ptr, reinterpret_cast<uint64_t*>(&shared_storage.full_barriers[stage_idx]),
                        static_cast<uint64_t>(cute::TMA::CacheHintSm100::EVICT_NORMAL),
                        shared_storage.smem_b[stage_idx], k_idx, n_idx);
                    tma::copy<BLOCK_N, 1, 0>(
                        tensor_map_sfb_ptr, &shared_storage.full_barriers[stage_idx], shared_storage.smem_sfb[stage_idx], sfb_n_idx, sfb_k_idx);
                    if (is_leader_cta)
                        shared_storage.full_barriers[stage_idx].arrive_and_expect_tx(
                            sizeof(SharedStorage::smem_b[0]) * 2 + sizeof(SharedStorage::smem_sfb[0]));
                    else
                        shared_storage.full_barriers[stage_idx].arrive_and_expect_tx(
                            sizeof(SharedStorage::smem_sfb[0]));
                }
                __syncwarp();
            }
        });
        ...
        if (is_leader_cta) {
            ...
            // NVFP4 uses UE4M3 block scale factors; MXFP4 uses UE8M0
            ...
            auto instr_desc = cute::UMMA::make_instr_desc_block_scaled<
                b_dtype_t, a_dtype_t, float, sf_dtype_t,
                UMMA_M, UMMA_N,
            ...
            // Packed-FP4 K-major smem layout (byte-addressed; swizzle == BLOCK_K/2 bytes)
            constexpr auto kFP4Layout = mma::sm100::to_umma_layout_type<cute::UMMA::Major::K, kSwizzleAMode, false, a_dtype_t>();
            ...
            scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
                ...
                #pragma unroll 2
                for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                    // Wait both CTAs' TMA loads (funneled to the leader by the sync warp)
                    shared_storage.with_sf_full_barriers[stage_idx].wait(phase);
                    ptx::tcgen05_after_thread_sync();
                    if (cute::elect_one_sync()) {
                        #pragma unroll
                        for (uint32_t umma_k_block_idx = 0; umma_k_block_idx < BLOCK_K / UMMA_BLOCK_K; ++ umma_k_block_idx) {
                            // UTCCP copy SFA and SFB to TMEM (SF pre-transposed during dispatch / weight transform).
                            // NVFP4 (gran-16) packs 2x the K-scale-factors per token (kNumSFKUint32PerUmmaBlock),
                            // laid out [K-uint][M] in smem and [M-group][K-uint][4 cols] in tmem.
                            using cute_utccp_t = cute::SM100_UTCCP_4x32dp128bit_2cta;
                            #pragma unroll
                            for (uint32_t ku = 0; ku < kNumSFKUint32PerUmmaBlock; ++ ku) {
                                #pragma unroll
                                for (uint32_t i = 0; i < SF_BLOCK_M / kNumUTCCPAlignedElems; ++ i) {
                                    auto smem_ptr = shared_storage.smem_sfa[stage_idx]
                                        + (umma_k_block_idx * kNumSFKUint32PerUmmaBlock + ku) * SF_BLOCK_M + i * kNumUTCCPAlignedElems;
                                    mma::sm100::replace_smem_desc_addr(sf_desc, smem_ptr);
                                    cute_utccp_t::copy(sf_desc, kTmemStartColOfSFA + (umma_k_block_idx * kNumSFKUint32PerUmmaBlock + ku) * kSFAColsPerKUint + i * 4);
                                }
                                #pragma unroll
                                for (uint32_t i = 0; i < SF_BLOCK_N / kNumUTCCPAlignedElems; ++ i) {
                                    auto smem_ptr = shared_storage.smem_sfb[stage_idx]
                                        + (umma_k_block_idx * kNumSFKUint32PerUmmaBlock + ku) * SF_BLOCK_N + i * kNumUTCCPAlignedElems;
                                    mma::sm100::replace_smem_desc_addr(sf_desc, smem_ptr);
                                    cute_utccp_t::copy(sf_desc, kTmemStartColOfSFB + (umma_k_block_idx * kNumSFKUint32PerUmmaBlock + ku) * kSFBColsPerKUint + i * 4);
                                }
                            }

                            // Issue packed-mxf4 UMMA over UMMA_K (=64) sub-tiles (byte-addressed smem)
                            const auto a_stage_ptr = reinterpret_cast<a_dtype_t*>(shared_storage.smem_a[stage_idx]) + umma_k_block_idx * UMMA_BLOCK_K_BYTES;
                            const auto b_stage_ptr = reinterpret_cast<b_dtype_t*>(shared_storage.smem_b[stage_idx]) + umma_k_block_idx * UMMA_BLOCK_K_BYTES;
                            #pragma unroll
                            for (uint32_t k = 0; k < UMMA_BLOCK_K / UMMA_K; ++ k) {
                                // 4 SFs pack into one TMEM word (one K-uint32). The 2-bit `sf_id`
                                // selects WITHIN that K-uint32 (<=3); crossing K-uint32s (gran-16 needs
                                // 2 per K-block) is done via the SF tmem ADDRESS, not sf_id.
                                const uint32_t global_sf_idx = (umma_k_block_idx * (UMMA_BLOCK_K / UMMA_K) + k) * (UMMA_K / kGranK);
                                ...
                                auto a_desc = mma::sm100::make_smem_desc(kFP4Layout, a_stage_ptr + k * UMMA_K_BYTES, 8 * kSwizzleAMode, 0);
                                auto b_desc = mma::sm100::make_smem_desc(kFP4Layout, b_stage_ptr + k * UMMA_K_BYTES, 8 * kSwizzleBMode, 0);
                                // Swap-AB: weights (b_desc) take the UMMA "A" slot + SFB
        ...
        // Cross-CTA loads-done sync warp (runs on BOTH CTAs).
        // Each CTA's per-CTA SM90 loads fill its own `full_barriers`; this warp waits that and
        // funnels arrival to the leader's `with_sf_full_barriers`, so the leader-only 2-SM MMA
        // only fires once BOTH CTAs' operands are resident. Mirrors the validated standalone.
        scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
                                     const uint32_t& local_expert_idx,
                                     const uint32_t& num_k_blocks,
                                     const uint32_t& m_block_idx, const uint32_t& n_block_idx) {
            for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                // All 32 lanes arrive on the leader CTA (init == 2 * 32)
                shared_storage.with_sf_full_barriers[stage_idx].arrive(0u);
            }
        });
            ...
            if (block_phase == sched::BlockPhase::Linear1) {
                ...
                for (uint32_t s = 0; s < WG_BLOCK_M / STORE_BLOCK_M; ++ s) {
                    ...
                    for (uint32_t i = 0; i < kNumAtomsPerStore; ++ i) {
                        ...
                        // Reduce amax (warp-pair-level) -- only MXFP4 (gran-32 SF spans 2 warps).
                        // NVFP4 (gran-16) has one SF per warp, so each warp keeps its own amax.
                        if constexpr (not kIsNVFP4) {
                            if (lane_idx < 4)
                    ...
                    #pragma unroll
                    for (uint32_t i = 0; i < kNumAtomsPerStore; ++ i) {
                        // Reduce amax (warp-pair-level), MXFP4 only
                        if constexpr (not kIsNVFP4) {
                        ...
                        // Output block SF + the inverse used to scale activations before the E2M1 cast.
                        // MXFP4: UE8M0 (pow2 of amax/6). NVFP4: E4M3 of amax/(6*gs), code = act/(sf_e4m3*gs).
                        float2 sf_inv;
                        ...
                        if constexpr (kIsNVFP4) {
                            ...
                            const int ey = math::fast_log2_ceil(scaled.y);
                            sf_inv.x = math::fast_pow2(-ex);
                            sf_inv.y = math::fast_pow2(-ey);
                            float2 sf;
                            sf.x = math::fast_pow2(ex);  sf.y = math::fast_pow2(ey);
                            sf_byte_x = static_cast<uint8_t>(*reinterpret_cast<const uint32_t*>(&sf.x) >> 23);
                            sf_byte_y = static_cast<uint8_t>(*reinterpret_cast<const uint32_t*>(&sf.y) >> 23);
                        }

                        // Cast 2 activation outputs -> e2m1 nibbles, then write them DIRECTLY to their
                        // [token][inter] packed positions (no stmatrix / no smem swizzle). This
                        // relies on the verified `SM100_TMEM_LOAD_16dp256b1x` fragment map for the
                        // (swap-AB) accumulator: with lane = (a = lane%4, b = lane/4) and warp w,
                        //   activation_values[i][0].{x,y} -> (token 2a / 2a+1, inter w*16 + b)
                        //   activation_values[i][1].{x,y} -> (token 2a / 2a+1, inter w*16 + 8 + b)
                        // Packed K-major requires 2 consecutive inter per byte, so the even-`b` lane
                        // fetches the odd inter nibble from its `b+1` partner (lane + 4) via shuffle.
                        ...
                        asm volatile(
                            ...
                            "}\n"
                            : "=h"(e2m1x4)
                            : "f"(upper.x), "f"(upper.y), "f"(lower.x), "f"(lower.y));

                        // My 4 nibbles (low bits): [upper.x | upper.y | lower.x | lower.y]
                        const uint32_t my_nibs =
                            ((uint32_t)(e2m1x4 & 0xF))             |
                            ((uint32_t)((e2m1x4 >> 4) & 0xF) << 8) |
                            ((uint32_t)((e2m1x4 >> 8) & 0xF) << 16)|
                            ((uint32_t)((e2m1x4 >> 12) & 0xF) << 24);
                        // Partner = lane + 4 (the `b+1` inter); its nibbles become the high half.
                        const uint32_t pr_nibs = __shfl_sync(0xffffffffu, my_nibs, lane_idx + 4);

                        // Only even-`b` lanes write (they own the even/low inter of each packed byte)
                        if (((lane_idx / 4) & 1) == 0) {
                            constexpr uint32_t L1_OUT_BLOCK_N_BYTES = L1_OUT_BLOCK_N / 2;
                            const uint32_t a = lane_idx % 4, bb = lane_idx / 4;
                            const uint32_t tok0 = i * ATOM_M + 2 * a;                 // local token in store tile
                            const uint32_t j0 = warp_idx_in_wg * 8 + bb / 2;          // byte for inter (w*16+bb, +1)
                            const uint32_t j1 = warp_idx_in_wg * 8 + 4 + bb / 2;      // byte for inter (w*16+8+bb, +1)
                            auto* p = reinterpret_cast<uint8_t*>(shared_storage.smem_d.l1[epilogue_wg_idx][tma_stage_idx]);
                            p[(tok0    ) * L1_OUT_BLOCK_N_BYTES + j0] = (my_nibs & 0xF)         | (((pr_nibs)       & 0xF) << 4);
                            p[(tok0 + 1) * L1_OUT_BLOCK_N_BYTES + j0] = ((my_nibs >> 8) & 0xF)  | (((pr_nibs >> 8)  & 0xF) << 4);
                            p[(tok0    ) * L1_OUT_BLOCK_N_BYTES + j1] = ((my_nibs >> 16) & 0xF) | (((pr_nibs >> 16) & 0xF) << 4);
                            p[(tok0 + 1) * L1_OUT_BLOCK_N_BYTES + j1] = ((my_nibs >> 24) & 0xF) | (((pr_nibs >> 24) & 0xF) << 4);
                        }

                        // Store SF to `l2_sf_buffer` (MN-major): MXFP4 UE8M0 byte, NVFP4 E4M3 byte.
                        // MXFP4 (gran-32): one SF per warp-pair (2 SFs per n-block, even warp writes).
                        // NVFP4 (gran-16): one SF per warp (4 SFs per n-block, every warp writes).
                        const bool sf_writer = kIsNVFP4 ? (lane_idx < 4)
                                                        : (warp_idx_in_wg % 2 == 0 and lane_idx < 4);
                        if (sf_writer) {
                            ...
                            sf_base_ptr[sf_addr] = sf_byte_x;
                            sf_base_ptr[sf_addr + 4 * static_cast<uint32_t>(sizeof(uint32_t))] = sf_byte_y;
                        }
                        __syncwarp();
                    }
                    ptx::sync_aligned(128, kEpilogueWGBarrierStartIdx + epilogue_wg_idx);
