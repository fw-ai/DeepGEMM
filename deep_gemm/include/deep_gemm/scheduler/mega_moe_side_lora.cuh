...
template <uint32_t BLOCK_M, uint32_t BLOCK_N, uint32_t BLOCK_K,
          ...
          bool kHasSideLora = false,
          uint32_t SIDE_BLOCK_K = BLOCK_K,
          uint32_t kNumExpertsPerLane = math::constexpr_ceil_div(kNumExpertsPerRank, 32u),
          uint32_t kNumL1BlockNs = L1_SHAPE_N / BLOCK_N,
          uint32_t kNumL2BlockNs = L2_SHAPE_N / BLOCK_N,
          uint32_t kNumL1BlockKs = L1_SHAPE_K / BLOCK_K,
          uint32_t kNumL2BlockKs = L2_SHAPE_K / BLOCK_K,
          uint32_t kNumLoraL1ShrinkBlockKs = L1_SHAPE_K / SIDE_BLOCK_K,
          uint32_t kNumLoraL2ShrinkBlockKs = L2_SHAPE_K / SIDE_BLOCK_K,
          uint32_t kNumLoraExpandBlockKs = 128 / SIDE_BLOCK_K>
struct SideLoraMegaMoEScheduler {
    ...
    DG_STATIC_ASSERT(L1_SHAPE_K % SIDE_BLOCK_K == 0, "Invalid side shape");
    DG_STATIC_ASSERT(L2_SHAPE_K % SIDE_BLOCK_K == 0, "Invalid side shape");
    DG_STATIC_ASSERT(128 % SIDE_BLOCK_K == 0, "Invalid side rank tile");
    ...
    // Rank-128 side LoRA maps naturally to the 2-CTA, 128-column Mega tile:
    // A1/A3 occupy the two shrink CTAs, B1/B3 alternate within each expand
    // pair, and A2 is duplicated across the pair (only CTA 0 is stored).
    static constexpr uint32_t kNumLoraL1ShrinkBlockNs = 2;
    static constexpr uint32_t kNumLoraL1ExpandBlockNs = kNumL1BlockNs;
    static constexpr uint32_t kNumLoraL2ShrinkBlockNs = 2;
    ...
    CUTLASS_DEVICE bool fetch_next_block_with_n_count(const uint32_t num_n_blocks) {
        ...
        while (current_local_expert_idx < wave_end_expert_idx) {
            ...
            block_idx -= num_m_blocks * num_n_blocks;
            advance_expert_idx();
        }
        return false;
    }
    ...
    CUTLASS_DEVICE uint32_t get_phase_num_k_blocks(const BlockPhase phase) const {
        ...
        return kNumL1BlockKs +
            // Gate and up have different shared-A projections.  A 2-CTA UMMA
            // splits M across the CTAs, so they must be issued as two K passes
            // rather than assigning q1/B1 to CTA 0 and q3/B3 to CTA 1.
    ...
    CUTLASS_DEVICE cute::tuple<BlockPhase, uint32_t, uint32_t, uint32_t> get_next_block() {
        while (true) {
            ...
            const auto num_n_blocks = get_phase_num_n_blocks(phase);
            if (fetch_next_block_with_n_count(num_n_blocks)) {
                n_block_idx = block_idx - m_block_idx * num_n_blocks;
                block_idx += kNumSMs;
                return {phase, current_local_expert_idx, m_block_idx, n_block_idx};
            } else {
    ...
    CUTLASS_DEVICE void for_each_block(Func&& func) {
        ...
        while (true) {
            ...
            func(block_phase, current_local_expert_idx,
                 get_phase_num_k_blocks(block_phase),
