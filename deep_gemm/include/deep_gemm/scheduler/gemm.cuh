...
template <GemmType kGemmType,
          ...
          uint32_t kKBlockSize = 128u,      // physical K tile (BF16 training also uses 16/32/64)
...
struct Scheduler {
    ...
    DG_STATIC_ASSERT(not is_k_grouped_contiguous(kGemmType) or kKAlignment % kKBlockSize == 0,
                     "K alignment must be a multiple of the physical K tile");
